...
public:
	...
	void Update()
	{
		...
		unsigned long deltaUs = nowUs - m_lastUs;
		m_lastUs = nowUs;

		m_toggleUsAccumulator += deltaUs;

		--m_analogReadDelay;
		if (m_analogReadDelay <= 0)
		{
			m_analogReadDelay = 10;
		...
		if (m_toggleUsAccumulator > m_toggleDeltaUs)
		{
			...
			digitalWrite(m_outputPin, m_currentValue);
		}
	}

private:
	int m_analogReadDelay;
	int m_inputPin;
	int m_outputPin;
	unsigned long m_lastUs;
	unsigned long m_toggleDeltaUs;
	unsigned long m_toggleUsAccumulator;
	bool m_currentValue;
};
